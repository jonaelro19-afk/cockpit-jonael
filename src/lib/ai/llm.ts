// Appel LLM générique (fournisseur au choix, dans cet ordre) :
//   1. Google Gemini   — GEMINI_API_KEY   (offre gratuite)
//   2. Anthropic Claude — ANTHROPIC_API_KEY (payant)
// Sans clé : AiError("no-key"). Le reste de l'app continue de fonctionner.
...
export async function callLLM(
  ...
  if (!gemini && !anthropic)
    throw new AiError("no-key", "Aucune clé IA configurée.");
